...
function loginValidation(event) {
    ...
    let User_Password = document.getElementById("loginpassword");
    let userPassValue = User_Password.value

    // Fetch signup data from localStorage
    let signupData = JSON.parse(localStorage.getItem("userData")) || [];

    let matchedUser = signupData.find(user => 
        user.email === User_Email && user.password === userPassValue
    );

    if (matchedUser) {
        // First matched user
        openSignupModal(matchedUser);
    } else {
